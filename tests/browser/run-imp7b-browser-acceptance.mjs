...
try{
  ...
  const accessible=async name=>{const missing=await page.evaluate(()=>[...document.querySelectorAll('main button,main input,main select,main textarea')].filter(e=>e.getClientRects().length&&e.type!=='hidden'&&!e.getAttribute('aria-label')&&!e.getAttribute('aria-labelledby')&&!e.labels?.length&&!(e.tagName==='BUTTON'&&e.textContent.trim())).map(e=>({tag:e.tagName,name:e.name,type:e.type})));check('CONTROL_LABELS_'+name,missing.length===0,JSON.stringify(missing));};
  await page.goto(base,{waitUntil:'domcontentloaded'});
  await page.keyboard.press('Tab');check('KEYBOARD_SKIP_LINK',await page.locator('.skip-link').evaluate(e=>e===document.activeElement));await page.keyboard.press('Enter');check('KEYBOARD_MAIN_FOCUS',await page.locator('main').evaluate(e=>e===document.activeElement));
  await page.locator('[data-upload-root]').waitFor();await accessible('upload');check('UPLOAD_SCREEN_RENDERED',true);
  ...
  await accessible('preparation');await shot('01-preparation');await page.locator('[data-validate]').focus();await page.keyboard.press('Enter');await page.locator('[data-validation-root]').waitFor();
  check('VALIDATION_WARNING_IS_NOT_BLOCKER',!(await page.locator('[data-calculate]').isDisabled()));await shot('02-validation');
  await accessible('validation');const started=Date.now();await page.locator('[data-calculate]').focus();await page.keyboard.press('Enter');await page.locator('[data-results-root]').waitFor({timeout:120000});evidence.goldenCalculationMs=Date.now()-started;
  ...
  const bundle=await download('[data-bulk]');check('BULK_ZIP',bundle.bytes.subarray(0,2).toString()==='PK');const combined=await download('[data-combined-pdf]');check('COMBINED_PDF',combined.bytes.subarray(0,5).toString()==='%PDF-');await shot('06-reports');await page.locator('[data-component]').selectOption('KPI-1.1.1');await page.locator('[data-status]').selectOption('accepted-certified');check('EMPTY_SCOPE_CSV_DISABLED',await page.locator('[data-export="csv"]').isDisabled());await page.locator('[data-reset-filters]').click();
  for(const [name,width,height] of [['tablet',834,1112],['mobile',390,844]]){await page.setViewportSize({width,height});for(const route of ['results','structures','assurance','reports','audit','review']){await page.locator('[data-route="'+route+'"]').click();await page.locator('[data-'+({results:'results-root',structures:'structures-root',reports:'reports-root',audit:'audit-root',assurance:'assurance-root',review:'validation-root'}[route])+']').waitFor();await accessible(name+'_'+route);await shot('07-'+name+'-'+route);const overflow=await page.evaluate(()=>({width:innerWidth,documentWidth:document.documentElement.scrollWidth,elements:[...document.querySelectorAll('body *')].filter(e=>!e.closest('.table-scroll')&&e.getBoundingClientRect().right>innerWidth+1).slice(0,15).map(e=>({tag:e.tagName,class:e.className,right:e.getBoundingClientRect().right}))}));check('NO_PAGE_OVERFLOW_'+name+'_'+route,overflow.documentWidth<=overflow.width+1,JSON.stringify(overflow));}}
  ...
  await page.reload({waitUntil:'domcontentloaded'});const leapBytes=await fs.readFile(path.join(root,'samples/leap/GOLDEN_E2E_LEAP_FY2023-24.xlsx'));const dt=await page.evaluateHandle(bytes=>{const d=new DataTransfer();d.items.add(new File([Uint8Array.from(atob(bytes),c=>c.charCodeAt(0))],'GOLDEN_E2E_LEAP_FY2023-24.xlsx',{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));return d;},leapBytes.toString('base64'));await page.locator('[data-drop-zone]').dispatchEvent('drop',{dataTransfer:dt});await page.locator('[data-review]').click();await page.locator('[data-continue]').click();await page.locator('[data-finalize-mapping]').click();await page.locator('[data-validate]').click();await page.locator('[data-calculate]').click();await page.locator('[data-results-root]').waitFor({timeout:120000});
  check('LEAP_RESULTS_RENDERED',await page.locator('[data-synthetic-badge]').isVisible());await page.locator('[data-route="structures"]').click();await page.locator('[data-detail-select]').selectOption('S-CCT-01');await page.locator('[data-open-detail]').click();const leapTrace=JSON.parse((await download('[data-trace-json]')).bytes);check('TRACE_366_ROWS',leapTrace.rows.length===366);await shot('08-leap-structure');
  check('NO_PROJECT_DATA_IN_URL',new URL(page.url()).search===''&&['','#main'].includes(new URL(page.url()).hash));
