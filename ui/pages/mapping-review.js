...
function drawerHtml(row,vm){
  const suggestions=(row.suggestions||[]).map(s=>`<li><strong>${esc(s.header)}</strong><span>${esc(Math.round(Number(s.score||0)*100))}% confidence · column ${esc(Number(s.columnIndex)+1)}</span></li>`).join('')||'<li class="muted">No protected E02 suggestions available.</li>';
  return `<aside class="mapping-drawer" data-mapping-drawer aria-label="Mapping detail"><div class="drawer-head"><div><span class="eyebrow">Controlled field</span><h2>${esc(row.controlledField)}</h2><p class="mono">${esc(row.fieldId)}</p></div><button class="button secondary compact-button" data-close-drawer>Close</button></div>
    ...
    <div class="drawer-section privacy-note"><strong>Source samples withheld on this surface.</strong><p class="muted">E02 candidate headers and confidence are shown. Raw workbook row values are not copied into Mapping Review product state.</p></div>
  </aside>`;
}
...
export async function mountMappingReview({main,controller,onBack=null,onConfirmed=null}={}){
  ...
  function render(errorMessage=''){
    ...
    main.innerHTML=`<div class="mapping-page" data-mapping-root>
      ...
      ${sheetAttention.length?`<section class="card"><div class="card-heading"><div><span class="eyebrow">Sheet mapping</span><h2>Sheet confirmations needing attention</h2></div><span class="status-pill warning">${sheetAttention.length}</span></div><div class="sheet-mapping-list">${sheetAttention.map(s=>`<div class="sheet-mapping-row"><div><strong>${esc(s.expectedSheet)}</strong><span class="cell-note">${esc(s.method)} · ${esc(s.confidenceLabel)}</span></div><select class="mapping-select" data-sheet-select="${esc(s.expectedSheet)}">${sheetOptions(vm.sourceCatalog,s.sourceSheet)}</select><div class="row-actions"><button class="button secondary compact-button" data-apply-sheet="${esc(s.expectedSheet)}">Apply</button>${s.sourceSheet&&s.needsConfirmation&&!s.confirmed?`<button class="button primary compact-button" data-confirm-sheet="${esc(s.expectedSheet)}">Confirm</button>`:''}</div></div>`).join('')}</div></section>`:''}
      <section class="card mapping-table-card"><div class="mapping-tabs" role="tablist" aria-label="Mapping filters">${vm.filters.map(t=>`<button type="button" class="mapping-tab" data-filter="${t.id}" aria-selected="${filter===t.id}">${esc(t.label)} <span>${t.count}</span></button>`).join('')}</div>
        <div class="table-scroll"><table class="data-table mapping-table"><caption class="sr-only">Controlled field mapping review</caption><thead><tr><th>Controlled field</th><th>Source</th><th>Method</th>${filter==='advanced'?'<th>Confidence</th><th>Requirement</th>':''}<th>Status</th><th></th></tr></thead><tbody>
        ${vm.rows.length?vm.rows.map(row=>`<tr class="mapping-row ${row.status.attention?'attention':''}"><td><strong>${esc(row.controlledField)}</strong><span class="cell-note mono">${esc(row.fieldId)} · ${esc(row.expectedSheet)}</span></td><td>${fieldSource(row)}</td><td>${esc(methodLabel(row.method))}${filter==='advanced'?`<span class="cell-note mono">${esc(row.method)}</span>`:''}</td>${filter==='advanced'?`<td>${esc(row.confidenceLabel)}</td><td>${esc(row.requirementClass)}</td>`:''}<td>${statusPill(row.status)}</td><td class="numeric"><button class="button secondary compact-button" data-open-field="${esc(row.fieldId)}">Review</button></td></tr>`).join(''):`<tr><td colspan="${filter==='advanced'?7:5}" class="muted">No mappings in this view.</td></tr>`}
        </tbody></table></div>
      </section>
      <details class="card advanced-details"><summary>Advanced mapping diagnostics</summary><div class="advanced-grid"><dl class="compact-meta"><div><dt>E02 contract</dt><dd class="mono">${esc(vm.engineContractVersion||'—')}</dd></div><div><dt>Schema</dt><dd class="mono">${esc(vm.schemaVersion||'—')}</dd></div><div><dt>Source session</dt><dd class="mono">${esc(vm.sourceSessionId||'—')}</dd></div><div><dt>Mapping status</dt><dd class="mono">${esc(vm.status)}</dd></div></dl><ul class="mapping-notes">${vm.notes.map(n=>`<li>${esc(n)}</li>`).join('')}</ul></div></details>
      <section class="preflight-actions"><button class="button secondary" data-back-mapping>Back to Preflight</button><div><span class="muted action-note">${vm.canConfirm?`All blocking mapping exceptions are resolved. ${vm.summary.unmappedOptional?`${vm.summary.unmappedOptional} optional field(s) remain unmapped and will stay null.`:''}`:'Resolve conflicts, Core unmapped fields and pending controlled confirmations before finalizing.'}</span><button class="button primary" data-finalize-mapping ${vm.canConfirm?'':'disabled aria-disabled="true"'}>Confirm mapping &amp; prepare data</button></div></section>
