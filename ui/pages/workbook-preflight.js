...
export function mountWorkbookPreflight({main,controller,onBack=null,onContinue=null}={}){
  ...
  <div class="preflight-page" data-preflight-root>
    ...
    <section class="card preflight-source-card">
      ...
      <dl class="compact-meta preflight-meta"><div><dt>Template</dt><dd>${esc(vm.template.detectedVersion||'Not detected')}</dd></div><div><dt>Reporting period</dt><dd>${esc(vm.reportingPeriod.label)}</dd></div><div><dt>Source fingerprint</dt><dd class="mono">${esc(vm.source.shortSha256||'Not available')}</dd></div><div><dt>Source session</dt><dd class="mono">${esc(vm.source.sourceSessionId||'Not available')}</dd></div></dl>
    </section>

    <section class="card">
      <div class="card-heading"><div><span class="eyebrow">Workbook structure</span><h2>Controlled sheet inventory</h2></div><span class="muted">${vm.sheetRows.length} listed</span></div>
      <div class="table-scroll"><table class="data-table"><caption class="sr-only">Controlled workbook sheet inventory</caption><thead><tr><th>Sheet</th><th>Status</th><th class="numeric">Rows</th><th class="numeric">Columns</th><th class="numeric">Formulas</th></tr></thead><tbody>
